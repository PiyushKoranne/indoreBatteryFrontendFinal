...
const OrdersBatch = () => {
	...
	async function handleProductDetails(product) {
		try {
			if(product.productSlug){
				navigate(`/buy-battery/${product?.productSlug}`)
			} else {
				const response = await axios.get(`https://batterybackend.react.stagingwebsite.co.in/api/v1/manage/get-slug/${product?.productId}`);
				if(response?.status === 200) {
					navigate(`/buy-battery/${response?.data?.slug}`)
				}
			}
		} catch (error) {
			console.log(error);
	...
	return (
		<>
			<Meta title="Order Batch | Indore Battery" />
			<div className='border-[1px] border-[rgba(0,0,0,0.1)] rounded-[8px] flex flex-col'>
				...
				{orderedItems?.map((product) => (
					<div key={product?.productId} className='p-[25px] pt-[10px] flex flex-col border-b-[1px] border-[rgba(0,0,0,0.1)]'>
						<h4 className='text-slate-600 font-["Sora"] text-[18px]'>Placed {`${months[new Date(orderLabel?.createdAt).getMonth()]} ${new Date(orderLabel?.createdAt).getDate()}, ${new Date(orderLabel?.createdAt).getFullYear()}`}</h4>
						<div className='flex items-start mt-[10px] h-[190px]'>
							...
							<div className='w-[85%] pl-[20px] flex flex-col items-start justify-between'>
								...
								<div className='flex items-center gap-[10px] mt-[25px]'>
									...
									<button onClick={()=>{handleProductDetails(product)}} className='rounded-[6px] bg-white shadow-md border-[1px] border-[rgba(0,0,0,0.2)] font-normal text-[12px] py-[6px] px-[12px]'>Product Details</button>
